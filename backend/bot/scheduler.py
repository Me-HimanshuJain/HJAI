...
from worker import celery_app  # Use the single shared Celery app
import asyncio
import logging
...
def fetch_daily_meetings():
    ...
    try:
        ...
        logger.error(f"Failed to fetch meetings (credentials missing?): {e}")
        return
...
def join_meeting_task(meeting_url: str, title: str):
    ...
    try:
        ...
        # Recipients should come from meeting attendees in a real setup
        recipients = os.getenv("RECAP_EMAIL_RECIPIENTS", "").split(",")
        recipients = [r.strip() for r in recipients if r.strip()]
        if recipients:
            distributor.send_recap_email(recipients, recap, {"title": title})
        else:
            logger.warning("No RECAP_EMAIL_RECIPIENTS set — skipping email send.")
